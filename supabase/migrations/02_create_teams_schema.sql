...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'teams' AND policyname = 'Lectura de equipos para usuarios autenticados'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'teams' AND policyname = 'Inserción de equipos para personal autorizado'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'teams' AND policyname = 'Modificación de equipos para personal autorizado'
  ) THEN
...
INSERT INTO public.teams (internal_code, name, category, sport, gender, season, status) VALUES
  ...
  ('EQU-000008', 'Cadete FEM', 'Cadete', 'Fútbol', 'FEMENINO', '2026/2027', 'ACTIVE'),
  ...
  ('EQU-000014', 'Infantil FEM', 'Infantil', 'Fútbol', 'FEMENINO', '2026/2027', 'ACTIVE'),
